...
# Goal -----------------------------------------------------------------------------------
#   Clean up the eGRID (2018) data: locations, fuel, capacity, etc.
...
# Load data: eGRID 2018 ------------------------------------------------------------------
  # Generator-level data
  gen18 = "egrid2018_data.xlsx" %>%
    here::here("DataRaw", "eGRID", .) %>%
    read_xlsx(sheet = "GEN18", skip = 1) %>%
    clean_names() %T>%
    setDT()
  # Drop 'seqgen18' == 0
  gen18 = gen18[seqgen18 != 0]
  # Plant-level data
  plant18 = "egrid2018_data.xlsx" %>%
    here::here("DataRaw", "eGRID", .) %>%
    read_xlsx(sheet = "PLNT18", skip = 1) %>%
    clean_names() %T>%
    setDT()

# Data work: Fuel cross-walk -------------------------------------------------------------
  # eGRID crosswalk between fuel and fuel category (using plant-level data)
  fuel_xwalk = plant18[, .(plprmfl, plfuelct)] %>% unique() %>% na.omit()
