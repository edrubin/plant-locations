...
# Goal -----------------------------------------------------------------------------------
#   Clean up the eGRID (2010) data: locations, fuel, capacity, etc.
...
# Load data: eGRID 2010 ------------------------------------------------------------------
  # Generator-level data
  gen10 = "egrid2010_data.xls" %>%
    here::here("DataRaw", "eGRID", .) %>%
    read_xls(sheet = "GEN10", skip = 5) %>%
    clean_names() %T>%
    setDT()
  # Plant-level data
  plant10 = "egrid2010_data.xls" %>%
    here::here("DataRaw", "eGRID", .) %>%
    read_xls(sheet = "PLNT10", skip = 4, na = c("NA", "", "N/A")) %>%
    clean_names() %T>%
    setDT()

# Data work: Fuel cross-walk -------------------------------------------------------------
# NOTE: This crosswalk is one of the ways pre-2010 (<=2010) data differ from post 2010
  # eGRID crosswalk between fuel and fuel category (using plant-level data)
  fuel_xwalk = plant10[, .(plprmfl, plfuelct)] %>% unique()
  # Drop missing 'plprmfl'
  fuel_xwalk %<>% .[!is.na(plprmfl)]
  # Change names
  setnames(fuel_xwalk, c("fuel_gen", "fuel_cat_gen"))
  # Load 2014 cross walk (eGRID does not have non-fossil categories in 2010)
  fuel_xwalk14 = here(
    "DataClean", "Plants", "All", "egrid-fuel-xwalk-2014.csv"
  ) %>% read_csv() %T>% setDT()
  setnames(fuel_xwalk14, old = "fuel_cat_gen", new = "fuel_cat_gen14")
  # Merge 2010 and 2014 crosswalks
  fuel_xwalk %<>% merge(
    y = fuel_xwalk14,
    by = "fuel_gen",
    all.x = T,
  ...
  fuel_xwalk[fuel_gen %in% c("BG", "DG", "MSB", "PP"), fuel_cat_gen := "BIOMASS"]
  fuel_xwalk[fuel_cat_gen == "OFSL", fuel_cat_gen := "OTHER FOSSIL"]
  fuel_xwalk = rbindlist(list(
    fuel_xwalk,
    data.table(
      fuel_gen = c("PG", "OTH", "PUR", "SGC", "MWH", "OBL", "SLW"),
      fuel_cat_gen = c("GAS", "OTHER", "OTHER", "COAL", "OTHER", "BIOMASS", "BIOMASS")
    )
...
# Data work: Fill missing plant types ----------------------------------------------------
  ...
  for (i in seq_along(fix_dt$oris)) {
    set(
      x = gen10,
      i = gen10[oris == fix_dt[i, oris], which = T],
      j = "fuel_cat_plant",
      value = fix_dt[i, fuel_cat_gen]
    )
  }
  # Drop 'is_missing' variable
  gen10[, is_missing := NULL]
  
# Data work: Flag small/closed generators ------------------------------------------------
  # Indicator for generators with more than 25 MW nameplate capacity
  gen10 = gen10[, i_big := 1 * (capacity_gen >= 25)]
  # Flag operating and stand-by units
  gen10 = gen10[, i_operating := 1 * (status_gen %in% c("OP", "SB"))]

# Save -----------------------------------------------------------------------------------
  # Save cleaned generator-level data
  write_fst(
    x = gen10,
    path = here("DataClean", "Plants", "All", "generator-egrid-all-2010.fst"),
    compress = 50
  )
