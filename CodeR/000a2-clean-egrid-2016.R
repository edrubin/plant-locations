...
# Goal -----------------------------------------------------------------------------------
#   Clean up the eGRID (2016) data: locations, fuel, capacity, etc.

# Notes ----------------------------------------------------------------------------------
#   - Create crosswalk forH fuel and category using eGRID plant data
#   - Flag big units (more than 25 MW nameplate capacity; drops 17,173 units)
#     This choice matches many programs (Google "25 MW EPA")
#       - New Source Performance Standards
#       - Acid Rain Program, 1999
#       - Cross-StateAir Pollution Rule
#       - MATS
#   - Keep only operating and stand-by units (drops 2,117)
...
# Load data: eGRID 2016 ------------------------------------------------------------------
  # Generator-level data
  gen16 = "egrid2016_data.xlsx" %>%
    here::here("DataRaw", "eGRID", .) %>%
    read_xlsx(sheet = "GEN16", skip = 1) %>%
    clean_names() %T>%
    setDT()
  # Plant-level data
  plant16 = "egrid2016_data.xlsx" %>%
    here::here("DataRaw", "eGRID", .) %>%
    read_xlsx(sheet = "PLNT16", skip = 1) %>%
    clean_names() %T>%
    setDT()

# Data work: Fuel cross-walk -------------------------------------------------------------
  # eGRID crosswalk between fuel and fuel category (using plant-level data)
  fuel_xwalk = plant16[, .(plprmfl, plfuelct)] %>% unique() %>% na.omit()
...
# Data work: Fill missing plant types ----------------------------------------------------
  ...
  for (i in seq_along(fix_dt$oris)) {
    set(
      x = gen16,
      i = gen16[oris == fix_dt[i, oris], which = T],
      j = "fuel_cat_plant",
      value = fix_dt[i, fuel_cat_gen]
    )
  }
  # Drop 'is_missing' variable
  gen16[, is_missing := NULL]
  
# Data work: Flag small/closed generators ------------------------------------------------
  # Indicator for generators with more than 25 MW nameplate capacity
  gen16 = gen16[, i_big := 1 * (capacity_gen >= 25)]
  # Flag operating and stand-by units 
  gen16 = gen16[, i_operating := 1 * (status_gen %in% c("OP", "SB"))]

# Save -----------------------------------------------------------------------------------
  # Save cleaned generator-level data
  write_fst(
    x = gen16,
    path = here("DataClean", "Plants", "All", "generator-egrid-all-2016.fst"),
    compress = 50
  )
  # Save cleaned generator-level data with only operating/standy-by units
  gen16 = gen16[(i_operating == 1) & (i_big == 1)]
  gen16 = gen16[, c("i_operating", "i_big") := NULL]
  write_fst(
    x = gen16,
    path = here("DataClean", "Plants", "generator-egrid-2016.fst"),
    compress = 50
  )
